...
2. Can we assume that having a genetic score of 0 makes one completely nonsusceptible to the environment (STRONG) or can individuals with a genetic score of 0 still be slightly susceptible (WEAK)?

See the figure below for a graphical representation of different types of interactions:
![](https://raw.githubusercontent.com/AlexiaJM/LEGIT/master/images/GxE_testing.png)

Vantage sensitivity is not presented here but it is just like diathesis-stress with a positive predictor. Note that the Figure assume only one genetic variant, but with the LEGIT model, we consider multiple genetic variants and the more genetic variants there are, the fewer there are individuals with a genetic score of 0.
...
where $c$ and $\beta_E$ depends on the model assumed. $c = 0$ represents diathesis-stress while $c \ne 0$ represents differential susceptibility. $\beta_E = 0$ assume a STRONG model while $\beta_E \ne 0$ assume a STRONG model.
...
When assuming diathesis-stress WEAK, we let $c=0$:
$$\mathbf{\mu} = 3 + (\mathbf{e}-0) + 2\mathbf{g}(\mathbf{e}-0) $$ 

We generate N=250 observations and test all 4 possible models based on the BIC (other choices are also available: AIC, AICc, cross-validation and cross-validated AUC). It is important to not include G or E in the model formula because they will be added automatically. In this case, we assume that we don't know the true minimum of the environment, thus we use option *crossover = "min"* to find the minimum of the observed data. 
```{r}
library(LEGIT)
...
```
Here we see the results. Notice that diathesis-stress WEAK is not the model with the lowest BIC. Differential susceptibility WEAK is slightly a better fit but the crossover point is not within observable range, thus we reject it.
...
```
This time, diathesis-stress WEAK is the best fit so we get the same conclusion.
Note that we only show the element *results* of the output, but the 4 models are also returned. 
...
When assuming diathesis-stress STRONG, we let $c=0$ and $\beta_E=0$:
$$\mathbf{\mu} = 3 + 2\mathbf{g}(\mathbf{e}-0) $$ 

We generate N=250 observations and test all 4 possible models based on the BIC assuming we don't know what is the minimum environemental score.
```{r}
...
When assuming differential susceptibility WEAK, we let $c=5$:
...
When assuming differential susceptibility STRONG, we let $c=5$ and $\beta_E=0$:
